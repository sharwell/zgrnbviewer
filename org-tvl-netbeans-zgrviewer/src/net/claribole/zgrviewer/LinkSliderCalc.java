/*
 * AUTHOR : Tomer Moscovich (tomer@moscovich.net) Copyright (c) INRIA, 2009. All
 * Rights Reserved Licensed under the GNU LGPL. For full terms see the file
 * COPYING.
 *
 * $Id: LinkSliderCalc.java 2105 2009-06-23 12:06:50Z rprimet $
 */
...
/**
 * Given a GeneralPath and a mouse cursor position, calculate path cursor
 * position and scale
 *
 * Usage: lsc = new LinkSliderCalc(edge.getPathReference(), myViewportWidth));
 * lsc.updateMousePosition(newMousePos); patPoint = lsc.getPositionAlongPath();
 * scale = lsc.getScale();
 */
public class LinkSliderCalc {
     ...
     * @param flatness	optional - min distance between curve control points and
     * flattened polyline representation
    ...
    /**
     * Call first to set the new mouse position. This calculates the new
     * position and scale factor along the path.
     *
     * @param mousePos
     */
    ...
    /**
     * @return Closest point on curve to mouse position (call
     * updateMousePosition first)
     */
    ...
    /**
     * @return fraction [0..1] closest point is along path (call
     * updateMousePosition first)
     */
    ...
    /**
     * @return tangent segment to curve at position along path (call
     * updateMousePosition first)
     */
    ...
    public static final class PolyLine {
         ...
         *
         * for(int i = 0; i < this.segments.size(); ++i){ Line2D seg1 =
         * this.segments.get(i); for(int j = 0; j < poly.segments.size(); ++j){
         * Line2D seg2 = poly.segments.get(j);
         * if(seg1.intersectsLine(poly.segments.get(j))){ //TODO intersects?
        ...
        public int intersectsCircleOnStartPoint(double r, double altR, Point2D intPt) {
            ...
            for (int i = 0; i < segments.size(); ++i) {
                if (center.distance(segments.get(i).getP2()) < r) {
                    continue;
                }
            ...
            for (int i = 0; i < segments.size(); ++i) {
                if (center.distance(segments.get(i).getP2()) < shortR) {
                    continue;
                }
        ...
        public static double segmentLength(Line2D seg) {
