/*
 * FILE: DotUtils.java DATE OF CREATION: Apr 4 2005 AUTHOR : Eric Mounhem
...
public class DotUtils {
    ...
    public static void readStyle(Style s, String v) {
        ...
        for (int i = 0; i < styles.length; i++) {
            ...
            for (value = 0; value < Style.styleAttributeName.length; value++) {
                found = styles[i].equalsIgnoreCase(Style.styleAttributeName[value]);
                if (found) {
                    break;
                }
            }
    ...
    public static void expandRects(Record r) {
        ...
        if (r.rects != null && r.subRecords != null) {
            rects = new ArrayList<Rectangle>(r.rects.length);
            rects.addAll(Arrays.asList(r.rects));
    ...
    private static List<Rectangle> expandRects(SubRecord sub, List<Rectangle> rects) {
        if (sub.label != null) {
            if (!sub.label.isEmpty()) {
                sub.rect = rects.remove(0);
            }
        }
