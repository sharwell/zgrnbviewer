/*
 * FILE: ZgrReader.java DATE OF CREATION: Apr 4 2005 AUTHOR : Eric Mounhem
 * (skbo@lri.fr) Copyright (c) INRIA, 2004-2010. All Rights Reserved Licensed
...
/**
 * Create the ZVTM Glyphs corresponding to a given Graph. This class is used
 * only with ZGRViewer. If you don't want to depend on ZVTM, remove this class.
 *
 * @author Eric Mounhem
 */
public class ZgrReader {
    ...
    private static void createRecord(Record record, VirtualSpaceManager vsm,
        ...
        if (record.pos != null) {
            ...
            if (lines != null) {
                 ...
                 * VRectangle
                 */ {
                    rect = new VRectangle(x2 + x1, y2 + y1, 1,
                                          x2 - x1, y2 - y1, fill);
                }
    ...
    private static void createBasicNode(BasicNode node,
        ...
        if (node.pos != null) {
            ...
            case BasicNode.POLYGON:
                ...
                /*
                 * float[] positions = new float[node.sides]; for (int j = 0; j
                 * < positions.length; j++) { positions[j] = 1; } nodeGlyph =
                 * new VShape(node.pos.coords[0], node.pos.coords[1], 1, (long)
                 * (inches * Math.min( node.width, node.height)), positions,
                 * fill, (float) (Math.PI / 4));
                     nodeGlyph.setBorderColor(border);
                 */
                break;
    ...
    private static Rectangle[] findRectsLines(Rectangle[] rects) {
        if (rects != null) {
            ...
            List<Rectangle> alllines = new ArrayList<Rectangle>(vlines.size());

            // Adding only lines inside the bounding rectangle
            for (int i = 0; i < vlines.size(); i++) {
                Rectangle test = vlines.get(i);
            ...
            for (int k = 0; k < texts.length; k++) {
                ...
                if (!text.isEmpty()) {
                    ...
                    VText g = new VText(x, y + 1 + texts.length * interline / 2
                        - (k + 1) * (interline - 1), 1, color, text.replaceAll("\\\\", ""), VText.TEXT_ANCHOR_MIDDLE);
