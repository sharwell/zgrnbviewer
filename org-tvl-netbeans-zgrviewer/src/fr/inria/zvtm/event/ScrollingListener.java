 ...
 * 2007-2010. All Rights Reserved Licensed under the GNU LGPL. For full terms
...
public class ScrollingListener extends ViewAdapter {
    ...
    /**
     * @param sl the ScrollLayer object that manages the scroll bar layer
     * associated with this event handler
     * @param scrollLayerIndex index of scroll layer in the view
     * @param controlledLayerIndex index of of controlled layer in the view
     * (layer containing camera controlled by the scroll bars)
     */
    ...
    public void press1(ViewPanel v, int mod, int jpx, int jpy, MouseEvent e) {
        ...
        if ((g = v.lastGlyphEntered()) != null) {
            if (g == sl.getVerticalSlider()) {
                dragMode = DRAGGING_VSLIDER;
            } else if (g == sl.getHorizontalSlider()) {
                dragMode = DRAGGING_HSLIDER;
            } else {
    ...
    public void click1(ViewPanel v, int mod, int jpx, int jpy, int clickNumber, MouseEvent e) {
        if (g != null) {
            if (g == sl.getUpButton()) {
                sl.moveUp();
            } else if (g == sl.getDownButton()) {
                sl.moveDown();
            } else if (g == sl.getLeftButton()) {
                sl.moveLeft();
            } else if (g == sl.getRightButton()) {
                sl.moveRight();
            }
        }
    }
    ...
    public void mouseDragged(ViewPanel v, int mod, int buttonNumber, int jpx, int jpy, MouseEvent e) {
        switch (dragMode) {
        case DRAGGING_VSLIDER: {
            sl.draggingVerticalSlider(lastJPY - jpy);
            break;
        }
        case DRAGGING_HSLIDER: {
            sl.draggingHorizontalSlider(jpx - lastJPX);
            break;
        }
        }
        lastJPX = jpx;
