...
public class FontDialog extends JDialog implements ActionListener {
    ...
    public static Font decode(String fontSpec) {
        if (fontSpec != null && fontSpec.length() > 0) {
            ...
            if (tokens.length >= 3) {
                ...
                int style;
                if (tokenizedStyle.equals("Bold")) {
                    style = Font.BOLD;
                } else if (tokenizedStyle.equals("Italic")) {
                    style = Font.ITALIC;
                } else if (tokenizedStyle.equals("BoldItalic")) {
    ...
    final void initUI() {//depending on selected item/default shape type
        Container cp = this.getContentPane();
        try {
            okBt.removeActionListener(this);
            cancelBt.removeActionListener(this);
        } catch (NullPointerException ex) {/*
             * all these might be null (for instance when poping up a GlyphFactory for the first time)
             */
        ...
        if (DEFAULT_STYLE == Font.PLAIN) {
            styleList.setSelectedValue("Plain", true);
        } else if (DEFAULT_STYLE == Font.BOLD) {
            styleList.setSelectedValue("Bold", true);
        } else if (DEFAULT_STYLE == Font.ITALIC) {
            styleList.setSelectedValue("Italic", true);
        } else if (DEFAULT_STYLE == Font.BOLD + Font.ITALIC) {
    ...
    Font getSelectedFont() {
        ...
        if (selectedStyle.equals("Bold")) {
            ...
            style = Font.ITALIC;
        } else if (selectedStyle.equals("BoldItalic")) {
            style = Font.BOLD + Font.ITALIC;
        } else {
            style = Font.PLAIN;
        }//"Plain"
    ...
    public static String getFontStyleName(int i) {
        if (i == java.awt.Font.BOLD) {
            return "Bold";
        } else if (i == java.awt.Font.ITALIC) {
            return "Italic";
        } else if (i == java.awt.Font.BOLD + java.awt.Font.ITALIC) {
            return "BoldItalic";
        } else {
            return "Plain";
        }
