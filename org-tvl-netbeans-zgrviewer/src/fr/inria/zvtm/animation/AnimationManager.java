...
public class AnimationManager {
    ...
    /**
     * Starts this AnimationManager. This method must be called for animations
     * to start. This method may be called multiple times from multiple threads;
     * only the first one will be taken in consideration, and subsequent calls
     * will have no effect.
     */
    public void start() {
    ...
    /**
     * Stops this AnimationManager After calling this method, all animations
     * handled by this AnimationManager will stop. This method must be called
     * once and only once, otherwise AnimationManager will complain loudly (i.e.
     * throw an IllegalThreadStateException). This AnimationManager should not
     * be used after stop() has been called.
     *
     ...
     * Starts an animation.
     *
     * @param anim animation to start (must have been created by calling
     * createAnimation on the same AnimationManager).
     * @param force if true, any previously started and conflicting animations
     * will be cancelled. Queued conflicting animations that did not yet start
     * are cancelled. Two animations conflict if they target the same subject
     * and animate the same dimension. Note that the end() action of a cancelled
     * animation will <b>not</b> be executed.
     */
    ...
    /**
     * Stops an animation. The end() action of an animation will be executed if
     * the animation has been started beforehand.
     *
     * @param anim animation to stop (must have been created by calling
     * createAnimation on the same AnimationManager).
     */
     ...
     *
     * @param anim animation to cancel (must have been created by calling
     * createAnimation on the same AnimationManager). Note that the end() action
     * of a cancelled animation will not be executed.
     */
    ...
    /**
     * Pauses an animation. An animation that is paused still prevents
     * conflicting animations to run.
     *
     * @param anim animation to pause
     * @return true if the animation was actually paused (ie was running prior
     * to the user call), false otherwise.
     */
    ...
    /**
     * Sets the timing events period, in milliseconds. This is not a guaranteed
     * resolution but rather a minimal value.
     *
     * @param resolution minimal tick interval, in milliseconds. All animations
     * handled by this AnimationManager share the same resolution.
     */
    ...
    /**
     * A class that represents an indefinite, interactive camera animation. The
     * user interacts with this animation by providing instantaneous camera
     * speeds.
     */
