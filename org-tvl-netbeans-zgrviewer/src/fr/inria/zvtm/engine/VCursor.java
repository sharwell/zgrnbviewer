/*
 * FILE: VCursor.java DATE OF CREATION: Jul 11 2000 AUTHOR : Emmanuel Pietriga
 ...
 *
 * <h4>Using DynaSpot</h4> <p>The DynaSpot behavior must be activated in
 * VCursor, calling</p> <ul><li>VCursor.getDynaPicker().activateDynaSpot(boolean
 * b)</li></ul>
 *
 * <p>In your ViewListener, simply call VCursor.dynaPick(Camera c) wherever this
 * makes sense. Usually this will be mouseMoved(...):</p> <ul>
 * <li>v.getCursor().getDynaPicker().dynaPick(c); // where c is the active
 * camera</li> </ul> <p>This updates the list of glyphs intersected by the
 * DynaSpot disc, and identifies the one glyph actually selected (which is
 * returned). The method also takes care of highlighting/unhighlighting the
 * selected glyph.</p> <p><strong>Note:</strong> dynaPick() also gets called
 * internally when DynaSpot's size changes.</p>
 */
public class VCursor {
    ...
    Color color;
    /**
     * color of geometrical hints associated with cursor (drag segment,
    ...
    boolean sync;
    /**
     * coord in camera space (same as jpanel coords, but conventional coord sys
     * at center of view panel, upward)
     */
    ...
    /**
     * Set whether this ZVTM cursor is synchronized with the system cursor or
     * not.
     */
    ...
    /**
     * Tells whether this ZVTM cursor is synchronized with the system cursor or
     * not.
     */
    ...
    /**
     * Set color of elements associated with cursor (drag segment, selection
     * rectangle, etc.).
     */
    ...
    /**
     * Unstick glyph that was last sticked to mouse. The glyph is automatically
     * made sensitive to mouse events. The number of glyphs sticked to the mouse
     * can be obtained by calling VCursor.getStickedGlyphsNumber().
     */
    ...
    /**
     * Tells whether entry/exit of cursor into/from glyphs is enabled or
     * disabled.
     */
    ...
    /**
     * Get the virtual space coordinates of the cursor. Unprojected w.r.t Camera
     * c.
     */
    ...
    void draw(Graphics2D g) {
        ...
        if (dynaPicker.dynaSpotActivated && dynaPicker.showDynarea) {
            ...
            case DynaPicker.DYNASPOT_VISIBILITY_VISIBLE: {
                g.setComposite(dynaPicker.dsST);
                break;
            }
            case DynaPicker.DYNASPOT_VISIBILITY_FADEIN: {
                g.setComposite(Translucency.acs[(int)Math.round((1 - dynaPicker.opacity) * dynaPicker.DYNASPOT_MAX_TRANSLUCENCY * Translucency.ACS_ACCURACY)]);
                break;
            }
            case DynaPicker.DYNASPOT_VISIBILITY_FADEOUT: {
                g.setComposite(Translucency.acs[(int)Math.round(dynaPicker.opacity * dynaPicker.DYNASPOT_MAX_TRANSLUCENCY * Translucency.ACS_ACCURACY)]);
                break;
            }
            }
