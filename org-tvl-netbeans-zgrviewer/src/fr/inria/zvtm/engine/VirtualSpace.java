/*
 * FILE: VirtualSpace.java DATE OF CREATION: Jul 11 2000 AUTHOR : Emmanuel
 * Pietriga (emmanuel.pietriga@inria.fr) MODIF: Emmanuel Pietriga
...
public class VirtualSpace {
    ...
    /**
     * Anonymous virtual space. Will generate a random name (guaranteed to be
     * unique).
     */
    public static final String ANONYMOUS = "AnonVS";

    /**
     * Compute the geometrical center of a set of glyphs (takes glyph sizes into
     * account)
     *
    ...
    /**
     * Remove camera at index i from this virtual space. When a camera is
     * destroyed, its index is not reused for another one. So if camera number
     * #3 is removed and then a new camera is added it will be assigned number
     * #4 even though there is no camera at index #3 any longer.
     *
     ...
     * Add glyph g to this virtual space.
     *
     * @param repaint pass false if views should not be repainted as a
     * consequence of this addition (default is true).
     */
    ...
    /**
     * Add a list of glyphs to this virtual space. glyphs with lower indices in
     * the list will be added first in the virtual space and will consequently
     * be lower in the drawing stack.
     *
     * @param repaint pass false if views should not be repainted as a
     * consequence of this addition (default is true).
     */
     ...
     * IMPORTANT: Read-only. Do not temper with this data structure unless you
     * know what you are doing. It is highly recommended to clone it if you want
     * to add/remove elements from it for your own purposes.
     */
    ...
    /*
     * Put glyph gl in the list of glyphs actually drawn. This list is used to
     ...
     * Get selected glyphs.
     */
    public List<Glyph> getSelectedGlyphs() {
        List<Glyph> v = new ArrayList<Glyph>();
        Glyph g;
        for (Iterator<Glyph> e = visualEnts.iterator(); e.hasNext();) {
            g = e.next();
            if (g.isSelected()) {
     ...
     */
    public List<Glyph> getGlyphsOfType(String t) {
        List<Glyph> v = new ArrayList<Glyph>();
        Glyph g;
        for (Iterator<Glyph> e = visualEnts.iterator(); e.hasNext();) {
            g = e.next();
            if ((t.isEmpty()) || (t.equals(g.getType()))) {
                v.add(g);
            }
        }
        return v;
    ...
    public void removeAllGlyphs(boolean repaint) {
        List<Glyph> entClone = new ArrayList<Glyph>(getAllGlyphs());
        for (int i = 0; i < entClone.size(); i++) {
            removeGlyph(entClone.get(i), false);
        }
        if (repaint) {
            VirtualSpaceManager.INSTANCE.repaint();
        }
    ...
    /**
     * Remove glyph g from this virtual space. ZVTM no longer holds a reference
     * to it. View will be updated.
     */
    ...
    /**
     * Remove this glyph from this virtual space. ZVTM no longer holds a
     * reference to it.
     *
     * @param repaint should the view be updated automatically or not once the
     * glyph has been removed. Default is true.
     */
    ...
    /**
     * show Glyph g <br>- use show() and hide() to change both the visibility
     * and sensitivity of glyphs <br>- use Glyph.setVisible() to only change the
     * glyph's visibility, but not its sensitivity.
     *
     * @see #hide(Glyph g)
     */
    public void show(Glyph g) {
        if (visualEnts.contains(g) && glyphIndexInDrawingList(g) == -1) {
            addGlyphToDrawingList(g);
        }
        VirtualSpaceManager.INSTANCE.repaint();
    }
    ...
    /**
     * hide Glyph g <br>- use show() and hide() to change both the visibility
     * and sensitivity of glyphs <br>- use Glyph.setVisible() to only change the
     * glyph's visibility, but not its sensitivity.
     *
     * @see #show(Glyph g)
    ...
    /**
     * Put this glyph after the last glyph that has z-index z, but before any
     * glyph that has a z-index higher than z (if any). g will be drawn after
     * every other glyph that has z-index z, but before any glyph that has a
     * z-index higher than z (if any).
     *
    ...
    /**
     * Put this glyph before the first glyph that has z-index z, but after any
     * glyph that has a z-index lower than z (if any). g will be drawn before
     * every other glyph that has z-index z, but after any glyph that has a
     * z-index lower than z (if any).
     *
     * @param z the considered z-index
     */
    ...
    /**
     * Put glyph g1 just above glyph g2 in the drawing list (g1 painted after
     * g2). Important: this might affect the g1's z-index.
     */
    ...
    /**
     * Put glyph g1 just below glyph g2 in the drawing list (g1 painted before
     * g2). Important: this might affect the g1's z-index.
     */
     ...
     * Get the bounding box of all glyphs visible in this virtual space.
     *
     * @param gl list of glyphs to iterate upon (will typically be a subset of
     * all glyphs in the virtual space)
     * @param res array which will contain the result
     * @return boundaries in VirtualSpace coordinates {west,north,east,south}
     */
        ...
        if (gl.length > 0) {
            ...
            for (int i = 1; i < gl.length; i++) {
                if (gl[i] instanceof RectangularShape) {
                    ...
                    tmp = gl[i].vx - rs.getWidth() / 2d;
                    if (tmp < res[0]) {
                        res[0] = tmp;
                    }
                    tmp = gl[i].vy + rs.getHeight() / 2d;
                    if (tmp > res[1]) {
                        res[1] = tmp;
                    }
                    tmp = gl[i].vx + rs.getWidth() / 2d;
    ...
    List<Picker> externalPickers = new ArrayList<Picker>(0);

    /**
     * Register an external picker with this view. The picker is said to be
     * <em>external</em> because it is not associated with a VCursor.
     *
     * @return true if the picker was not already registered with this view,
     * false if it was.
     */
    ...
    /**
     * Unregister an external picker from this view. The picker is said to be
     * <em>external</em> because it is not associated with a VCursor.
     *
     * @return true if the picker was registered with this view and did get
     * unregistered, false if it was not.
     */
