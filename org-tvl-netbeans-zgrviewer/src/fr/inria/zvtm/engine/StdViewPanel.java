/*
 * FILE: StdViewPanel.java DATE OF CREATION: Jul 11 2000 AUTHOR : Emmanuel
 * Pietriga (emmanuel.pietriga@xrce.xerox.com) MODIF: Emmanuel Pietriga
 ...
 *
 * $Id: StdViewPanel.java 4590 2011-08-26 13:38:11Z epietrig $
 */
...
/**
 * JPanel used to paint the content of a view (all camera layers). Manual double
 * buffering, to allow modifications to the rendering before it gets painted on
 * screen (e.g., applying magnification lenses).
 *
...
public class StdViewPanel extends ViewPanel {
    ...
    StdViewPanel(List<Camera> cameras, View v, boolean arfome) {
        panel = new PanelImpl();
        ...
        setAWTCursor(Cursor.CROSSHAIR_CURSOR);  //custom cursor means VTM cursor
                ...
                for (int i = 0; i < gll.length; i++) {
                    if (gll[i] != null) {
                        if (gll[i].visibleInViewport(viewWC, viewNC, viewEC, viewSC, cams[nbcam])) {
                            /*
                             * if glyph is at least partially visible in the
                             * reg. seen from this view, compute in which buffer
                            ...
                            if (drawLens) {
                                if (gll[i].visibleInViewport(lviewWC, lviewNC, lviewEC, lviewSC, cams[nbcam])) {
                                    /*
                                     * partially within the region seen through
                                     * the lens draw it in both buffers
                                     */
                            ...
                            /*
                             * notifying outside of above test because glyph
                             * sensitivity is not affected by glyph visibility
                             * when managed through Glyph.setVisible()
                             */
                ...
                if (repaintASAP) {
                    try {
                        ...
                        if (VirtualSpaceManager.debugModeON()) {
                            ...
                            Exceptions.printStackTrace(ex0);
                        }
                    }
        ...
        public void paint(Graphics g) {
            if (backBuffer != null) {
                ...
                if (repaintListener != null) {
                    repaintListener.viewRepainted(StdViewPanel.this.parent);
                }
            }
        }
    }
