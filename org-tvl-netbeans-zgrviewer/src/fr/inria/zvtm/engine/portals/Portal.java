/*
 * FILE: Portal.java DATE OF CREATION: Sat Jun 17 07:19:59 2006 AUTHOR :
...
public abstract class Portal {
    ...
    /**
     * Set an event handler for mouse and keyboard events occuring inside the
     * portal.
     */
    ...
    /**
     * Get the event handler for mouse and keyboard events occuring inside the
     * portal (null if none)
     */
     ...
     * @param cx cursor x-coordinate (JPanel coordinates system)
     * @param cy cursor y-coordinate (JPanel coordinates system)
     * @return 1 if cursor has entered the portal, -1 if it has exited the
     * portal, 0 if nothing has changed (meaning it was already inside or
     * outside it)
     */
    ...
    /**
     * Computes the distance between a given point (typically the mouse cursor)
     * and each of the four portal borders. The returned array contains the
     * following for distances: <ul> <li>Horizontal distance from left
     * border</li> <li>Vertical distance from top border</li> <li>Horizontal
     * distance from right border</li> <li>Vertical distance from bottom
     * border</li> </ul> <p>In all four cases, the value is positive if the
     * point is on the side of the border that potentially makes it belong to
     * the inside of the portal (<em>potentially</em> because the point has to
     * meet the requirement for all four borders to actually <em>be</em> inside
     * the portal). The value is zero when the point is exactly above the
     * border, and negative otherwise.</p> <p>This method usually makes the
     * hypothesis that the portal's shape is rectangular. If it is not, the
     * portal's bounding box is considered. Subclasses willing to change this
     * behaviour should be free to override this method.</p>
     *
     * @param cx horizontal cursor coordinate (JPanel)
     * @param cy vertical cursor coordinate (JPanel)
     * @param res the array to fill with information (to be initialized by
     * client application)
     * @return the projected distance between the provided coordinates and the
     * portal's borders
     */
