/*
 * FILE: CameraPortal.java DATE OF CREATION: Sat Jun 17 07:19:59 2006 AUTHOR :
 * Emmanuel Pietriga (emmanuel.pietriga@inria.fr) MODIF: Emmanuel Pietriga
 * (emmanuel.pietriga@inria.fr) Copyright (c) INRIA, 2004-2010. All Rights
...
/**
 * A portal showing what is seen through a camera. Shape: rectangular. The
 * Camera should not be used in any other View or Portal.
 */
public class CameraPortal extends Portal {
     ...
     *
     * @param x top-left horizontal coordinate of portal, in parent's JPanel
     * coordinates
     * @param y top-left vertical coordinate of portal, in parent's JPanel
     * coordinates
    ...
    /**
     * Builds a new possibly translucent portal displaying what is seen through
     * a camera
     *
     * @param x top-left horizontal coordinate of portal, in parent's JPanel
     * coordinates
     ...
     * @param h portal height
     * @param c camera associated with the portal
     * @param a alpha channel value (translucency). alpha ranges between 0.0
     ...
     * to which the camera associated with this portal belongs.
     *
    ...
    /**
     * Get bounds of rectangular region of the VirtualSpace seen through this
     * camera portal.
     *
    ...
    /**
     * Get the location from which this portal's camera will see all glyphs
     * visible in the associated virtual space.
     *
    ...
    /**
     * Translates and (un)zooms this portal's camera in order to see everything
     * visible in the associated virtual space.
     *
    ...
    public Location getGlobalView(int d) {
        ...
        Animation trans =
            VirtualSpaceManager.INSTANCE.getAnimationManager().getAnimationFactory().createCameraTranslation(d, camera, new Point2D.Double(l.vx, l.vy), false,
                                                                                                             IdentityInterpolator.getInstance(),
                                                                                                             null);
        Animation altAnim =
            VirtualSpaceManager.INSTANCE.getAnimationManager().getAnimationFactory().createCameraAltAnim(d, camera, l.alt, false,
                                                                                                         IdentityInterpolator.getInstance(),
                                                                                                         null);
    ...
    /**
     * Position this portal's camera so that it seamlessly integrates with the
     * surrounding context.
     *
    ...
    /**
     * Get the location from which this portal's camera will focus on a specific
     * rectangular region.
     *
    ...
    /**
     * Translates and (un)zooms this portal's camera in order to focus on a
