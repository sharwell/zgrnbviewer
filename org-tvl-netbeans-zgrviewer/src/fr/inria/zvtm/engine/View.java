/*
 * FILE: View.java DATE OF CREATION: Jul 11 2000 AUTHOR : Emmanuel Pietriga
 * (emmanuel.pietriga@xrce.xerox.com) MODIF: Emmanuel Pietriga
 ...
 *
 * $Id: View.java 4339 2011-03-08 15:22:49Z epietrig $
 */
...
/**
 * A view is a window and can be composed of one or several cameras superimposed
 * - use EView or IView <BR> A view is repainted on a regular basis when active
 * - for inactive views, the default is to repaint only if the mouse is inside
 * the view (but the frame is not selected) - this can be changed to repaint the
 * view automatically even if it is not selected and if the mouse is not inside,
 * using setRepaintPolicy()
 *
...
public abstract class View {
    ...
    /**
     * ZVTM view based on Java 5's OpenGL rendering pipeline; does accelerate
     * rendering but requires a JVM 1.5 or later. The use of OPENGL_VIEW
     * requires the following Java property: -Dsun.java2d.opengl=true
     */
    public static final String OPENGL_VIEW = "ogl";

    /**
     * Declare a new type of panel to draw on in a View. Two panel types are
     * available in zvtm-core: <ul> <li>View.STD_VIEW:double-buffered panel,
     * drawing in an offscreen BufferedImage, supports lenses</li>
     * <li>View.OPENGL_VIEW: direct rendering on screen, works with Sun's OpenGL
     * Java2D rendering pipeline </li> </ul>
     *
     * @param ptID unique ID identifying the new panel type. This is the ID
     * passed to view creation methods (viewType) to specifiy the type of view.
     * @param pf a class that can make new instances of the corresponding
     * ViewPanel, to be included in the View.
    ...
    /**
     * Two panel types/factories are available in zvtm-core: <ul>
     * <li>View.STD_VIEW:double-buffered panel, drawing in an offscreen
     * BufferedImage, supports lenses</li> <li>View.OPENGL_VIEW: direct
     * rendering on screen, works with Sun's OpenGL Java2D rendering pipeline
     * </li> </ul>
     *
     * @param ptID unique ID identifying the new panel type. This is the ID
     * passed to view creation methods (viewType) to specifiy the type of view.
     * @return an instance of PanelType that can make new instances of the
     * corresponding ViewPanel, to be included in the View.
     */
    public static PanelFactory getPanelFactory(String ptID) {
        return VIEW_TYPES.get(ptID);
    }

    static Map<String, PanelFactory> VIEW_TYPES = new HashMap<String, PanelFactory>(2);
    ...
    void initCameras(List<Camera> c) {
        cameras = c;
        for (int i = 0; i < cameras.size(); i++) {
            c.get(i).setOwningView(this);
        }
    }
    ...
    /**
     * Sets the active repaint interval for this View. By default, active
     * repaint is disabled.
     *
     * @param timeMillis active repaint interval, in milliseconds. if <= 0,
     * active repaint will be disabled.
     */
    ...
    ViewPanel panel;
    /**
     * enables detection of multiple full fills in one view repaint - for this
     * specific view - STILL VERY BUGGY - ONLY SUPPORTS VRectangle and VCircle
     * for now
     */
    ...
    /**
     * triggers the mouseMoved method in ViewListener when the mouse is moved -
     * set to false by default because few applications will need this; it is
     * therefore not necessary to overload other applications with these events
     */
    boolean notifyCursorMoved = true;
    /**
     * hooks for Java2D painting in ZVTM views (BACKGROUND, FOREGROUND,
     * AFTER_DISTORTION, AFTER_PORTALS)
     */
    ...
    /**
     * Set the cursor for this View. Either the ZVTM cursor or one of the
     * default AWT cursors.
     *
     * @param cursorType any of the cursor type values declared in
     * java.awt.Cursor, such as DEFAULT_CURSOR, CROSSHAIR_CURSOR HAND_CURSOR,
     * etc. To get the ZVTM cursor, use Cursor.CUSTOM_CURSOR.
    ...
    /**
     * Set the cursor for this view. Replaces the ZVTM cursor by a bitmap cursor
     * similar to the default AWT cursors.
     *
     * @param c an AWT cursor instantiated e.g. by calling
     * java.awt.Toolkit.createCustomCursor(Image cursor, Point hotSpot, String
     * name)
    ...
    /**
     * Set application class instance to which events are sent for all layers in
     * this view.
     *
    ...
    /**
     * Set application class instance to which events are sent for a given
     * layer.
     *
     * @param eh client application implementation of ViewListener
     * @param layer depth of layer to which the event handler should be
     * associated.
     */
    ...
    /**
     * Sets the ViewListener instance to which events are sent for a given
     * layer.
     *
     * @param eh client application implementation of ViewListener
    ...
    /**
     * Sets whether the mouseMoved callback in ViewListener is triggered when
     * the cursor moves. Set to true by default. Applications that do not care
     * about this callback can disable notification about these events to avoid
     * unnecessary callbacks (an event each sent each time the cursor moves).
     */
    ...
    /**
     * Tells whether the mouseMoved callback in ViewListener is triggered when
     * the mouse is moved. Set to true by default.
     */
    public boolean getNotifyCursorMoved() {
        return notifyCursorMoved;
    }
    ...
    /**
     * Tells whether detection of multiple full fills in one view repaint is
     * enabled or not for this View. Off by default.
     *
    ...
    /**
     * Get bounds of rectangular region of the VirtualSpace seen through a
     * camera.
     *
     * @param c camera
    ...
    /**
     * Get width of rectangular region of the VirtualSpace seen through a
     * camera.
     *
     * @param c camera
    ...
    /**
     * Get height of rectangular region of the VirtualSpace seen through a
     * camera.
     *
     * @param c camera
     * @return width in VirtualSpace coordinates (from north to south
     * boundaries)
     */
     ...
     *
     * @param i layer index. 0 is the deepest layer (first camera given in the
     ...
     *
     * @param cam a
     * <code>Camera</code> that represents the active layer.
     */
     ...
     *
     * @return layer index. 0 is the deepest layer (first camera given in the
     * Vector at construction time).
     */
     ...
     *
     * @param t - the title to be displayed in the frame's border. A null value
     ...
     *
     * @param c blank color (will fill the entire view) - pass null to exit
     * blank mode.
     */
    ...
    /**
     * Activating the view means that repainting will be enabled on it. This
     * triggers the viewActivated(View v) callback for the ViewListener
     * associated with the active camera layer in this view.
     *
    ...
    public void activate() {
        ...
        if (panel.evHs[panel.activeLayer] != null) {
            panel.evHs[panel.activeLayer].viewActivated(this);
        }
    }

    /**
     * Deactivating the view means that repainting will be disabled on it. This
     * triggers the viewDeactivated(View v) callback for the ViewListener
     * associated with the active camera layer in this view.
     *
    ...
    public void deactivate() {
        if (panel.evHs[panel.activeLayer] != null) {
            panel.evHs[panel.activeLayer].viewDeactivated(this);
        }
    }

    /**
     * Called from the window listener when the window is iconified. Repainting
     * is automatically disabled. This triggers the viewIconified(View v)
     * callback for the ViewListener associated with the active camera layer in
     * this view.
     *
    ...
    void iconify() {
        ...
        if (panel.evHs[panel.activeLayer] != null) {
            panel.evHs[panel.activeLayer].viewIconified(this);
        }
    }

    /**
     * Called from the window listener when the window is deiconified.
     * Repainting is automatically re-enabled. This triggers the
     * viewIconified(View v) callback for the ViewListener associated with the
     * active camera layer in this view.
     *
     * @see ViewListener#viewIconified(View v)
     */
    void deiconify() {
        panel.repaintable = true;
        if (panel.evHs[panel.activeLayer] != null) {
            panel.evHs[panel.activeLayer].viewDeiconified(this);
        }
    }

    /**
     * Called from the window listener when the window is closed. Repainting is
     * automatically re-enabled. This triggers the viewClosing(View v) callback
     * for the ViewListener associated with the active camera layer in this
     * view.
     *
    ...
    protected void close() {
        if (panel.evHs[panel.activeLayer] != null) {
            panel.evHs[panel.activeLayer].viewClosing(this);
        }
    }
     ...
     * Ask for the view to be repainted. This is an asynchronous call.
     *
     * @param rl a repaint listener, to get notified when this repaint cycle is
     * completed. The listener must be removed manually if you are not
    ...
    /**
     * Get access to the panel's AWT Graphics object. This can be useful in some
     * cases, for instance to compute the bounds of a text string that has not
     * yet been added to any virtual space. This instance of Graphics should not
     * be tampered with (this will be at your own risks).
     */
    ...
    /**
     * Get an image of what is visible in this view, by calling getImage on the
     * JComponent.
     *
     * @return a copy of the original offscreen buffer.
     * @see #rasterize(int w, int h)
     * @see #rasterize(int w, int h, List layers)
     */
    ...
    /**
     * Ask for a bitmap rendering of this view and return the resulting
     * BufferedImage.
     *
     * @param w width of rendered image
     * @param h height of rendered image
     * @return the resulting buffered image which can then be manipulated and
     * serialized
     * @see #rasterize(int w, int h, List layers)
     * @see #getImage()
     */
    public BufferedImage rasterize(int w, int h) {
        return rasterize(w, h, (List<Camera>)null);
    }
     ...
     * @param w width of rendered image
     * @param h height of rendered image
     * @param layers Vector of cameras : what layers (represented by cameras) of
     * this view should be rendered (you can pass null for all layers)
     * @return the resulting buffered image which can then be manipulated and
     * serialized
     * @see #rasterize(int w, int h)
     * @see #getImage()
     */
    public BufferedImage rasterize(int w, int h, List<Camera> layers) {
        Dimension panelSize = panel.getComponent().getSize();
        float mFactor = 1 / Math.min(w / ((float)panelSize.getWidth()),
                                     h / ((float)panelSize.getHeight()));
        Camera c, nc;
        List<Camera> clones = new ArrayList<Camera>();
        List<Camera> cams = (layers != null) ? layers : cameras;
    ...
    /**
     * Set a paint method (containing Java2D paint instructions) that will be
     * called each time the view is repainted.
     *
     * @param p the paint method encapsulated in an object implementing the
     * Java2DPainter interface (pass null to unset an existing one)
     * @param g one of Java2DPainter.BACKGROUND, Java2DPainter.FOREGROUND,
     * Java2DPainter.AFTER_DISTORTION, Java2DPainter.AFTER_PORTALS depending on
     * whether the method should be called before or after ZVTM glyphs have been
     * painted, after distortion by a lens (FOREGROUND and AFTER_DISTORTION are
     * equivalent in the absence of lens), or after portals have been painted
     */
    ...
    /**
     * Get the implementation of the paint method (containing Java2D paint
     * instructions) that will be called each time the view is repainted.
     *
     * @param g one of Java2DPainter.BACKGROUND, Java2DPainter.FOREGROUND,
     * Java2DPainter.AFTER_DISTORTION, Java2DPainter.AFTER_PORTALS depending on
     * whether the method should be called before or after ZVTM glyphs have been
     * painted, after distortion by a lens (FOREGROUND and AFTER_DISTORTION are
     * equivalent in the absence of lens), or after portals have been painted
     * @return p the paint method encapsulated in an object implementing the
     * Java2DPainter interface (null if not set)
     */
    ...
    /**
     * Set a padding for customizing the region inside the view for which
     * objects are actually visible.
     *
     * @param wnesPadding padding values in pixels for the west, north, east and
     * south borders
     */
    ...
    /**
     * Get the padding values customizing the region inside the view for which
     * objects are actually visible.
     *
     * @return padding values in pixels for the west, north, east and south
     * borders
     */
     ...
     */
    /**
     * Activate a lens in this view. This only works with regular views (not
     * OpengGL views - GLEView).
     *
     ...
     */
    /**
     * Get the location from which a camera will see all glyphs visible in the
     * associated virtual space. The camera must be used in this view.
     * Otherwise, the method returns null and does nothing.
     *
     * @param c camera considered (will not be moved)
     * @param mFactor magnification factor - 1.0 (default) means that the glyphs
     * will occupy the whole screen. mFactor &gt; 1 will zoom out from this
     * default location. mFactor &lt; 1 will do the opposite
     * @return the location to which the camera should go, null if the camera is
     * not associated with this view.
    ...
    public Location getGlobalView(Camera c, float mFactor) {
         ...
         * (need to compute zoom) ; we only take left and down because we only
         * need horizontal and vertical ratios, which are equals for left and
         * right, up and down
         */
    ...
    /**
     * Translates and (un)zooms a camera in order to see everything visible in
     * the associated virtual space. The camera must be used in this view.
     * Otherwise, the method returns null and does nothing.
     *
     * @param c Camera to be moved (will actually be moved)
     * @param d duration of the animation in ms
     * @return the final camera location, null if the camera is not associated
     * with this view.
     * @see #getGlobalView(Camera c)
     * @see #getGlobalView(Camera c, int d, float mFactor)
    ...
    /**
     * Get the location from which a camera will see everything visible in the
     * associated virtual space. The camera must be used in this view.
     * Otherwise, the method returns null and does nothing.
     *
     * @param c camera considered (will not be moved)
    ...
    /**
     * Translates and (un)zooms a camera in order to focus on glyph g The camera
     * must be used in this view. Otherwise, the method returns null and does
     * nothing.
     *
     ...
     * @param mFactor magnification factor: 1.0 (default) means that the glyph
     * will occupy the whole screen. mFactor > 1 will make the glyph smaller
     * (zoom out). mFactor < 1 will make the glyph appear bigger (zoom in).
     * @param endAction end action to execute after camera reaches its final
     * position
     * @return the final camera location, null if the camera is not associated
     * with this view.
     */
    ...
    /**
     * Translates and (un)zooms a camera in order to focus on a specific
     * rectangular region The camera must be used in this view. Otherwise, the
     * method returns null and does nothing.
     *
     * @param c Camera to be moved
     * @param d duration of the animation in ms (pass 0 to go there
     * instantanesouly)
    ...
    /**
     * Get glyphs whose hotspot is in region delimited by rectangle
     * (x1,y1,x2,y2) in VirtualSpace vs. Coordinates of the mouse cursor in
     * virtual space are available in instance variables vx and vy of class
     * VCursor. The selection rectangle can be drawn on screen by using
     * ViewPanel.setDrawRect(true) (e.g. call when mouse button is
     * pressed)/ViewPanel.setDrawRect(false) (e.g. call when mouse button is
     * released).
     *
     ...
     * @param wg which glyphs in the region should be returned (among
     * VIS_AND_SENS_GLYPHS (default), VISIBLE_GLYPHS, SENSIBLE_GLYPHS,
     * ALL_GLYPHS)
     */
    public List<Glyph> getGlyphsInRegion(double x1, double y1, double x2, double y2, String vsn, int wg) {
        List<Glyph> res = new ArrayList<Glyph>();
        ...
        double maxY = Math.max(y1, y2);
        if (vs != null) {
            List<Glyph> allG = vs.getAllGlyphs();
            Glyph g;
            for (int i = 0; i < allG.size(); i++) {
                g = allG.get(i);
                if ((g.vx >= minX) && (g.vy >= minY) && (g.vx <= maxX) && (g.vy <= maxY)) {
                    if ((wg == VirtualSpaceManager.VIS_AND_SENS_GLYPHS) && g.isSensitive() && g.isVisible()) {
                        res.add(g);
                    } else if ((wg == VirtualSpaceManager.VISIBLE_GLYPHS) && g.isVisible()) {
                        res.add(g);
                    } else if ((wg == VirtualSpaceManager.SENSITIVE_GLYPHS) && g.isSensitive()) {
                        res.add(g);
                    } else if (wg == VirtualSpaceManager.ALL_GLYPHS) {
                        res.add(g);
                    }
                }
            }
