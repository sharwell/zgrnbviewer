...
public class Picker {
    ...
    /**
     * List of glyphs picked at the pickers coordinates. Last entry is last
     * glyph entered. IMPORTANT: elements beyond maxIndex might not be up to
     * date. Do not trust the value, especially if not null.
     */
    Glyph[] pickedGlyphs;
    /**
     * Last glyph picked.
    ...
    /**
     * Picker constructor. Instantiated with a default stack size of 50.
     */
    public Picker() {
        this(50);
    }
     ...
     * Picker constructor.
     *
     * @param stackSize start picked glyph stack size. Will double capacity if
     * overflows.
     */
     ...
     *
     * @param c should be the active camera (can be obtained by
     * VirtualSpaceManager.getActiveCamera())
     * @param tolerance the rectangular area's half width/height considered as
     * the cursor intersecting region, in virtual space units (default tolerance
     * is 5)
     ...
     */
    public List<DPath> getIntersectingPaths(Camera c, int tolerance, double x, double y) {
        List<DPath> res = new ArrayList<DPath>();
        List<Glyph> glyphs = c.getOwningSpace().getDrawnGlyphs(c.getIndex());
        Glyph glyph;
        for (int i = 0; i < glyphs.size(); i++) {
            glyph = glyphs.get(i);
            if ((glyph instanceof DPath) && intersectsPath((DPath)glyph, tolerance, x, y)) {
                res.add((DPath)glyph);
            }
        }
        return res;
     ...
     * VirtualSpaceManager.getActiveCamera())
     * @return an empty vector if none
     * @see #getIntersectingPaths(Camera c, int tolerance, double cursorX,
     * double cursorY)
     */
    public List<DPath> getIntersectingPaths(Camera c) {
     ...
     * double cursorY)
     */
    public List<DPath> getIntersectingPaths(Camera c, int tolerance) {
     ...
     * the cursor intersecting region, in virtual space units (default tolerance
     * is 5)
     * @see #intersectsPath(DPath p, int tolerance, double cursorX, double
     * cursorY)
     */
     ...
     * @param tolerance the segment's abstract thickness (w.r.t picking) in
     * pixels, not virtual space units (we consider a narrow rectangular region,
     * not an actual segment)
     * @return an empty vector if none
     ...
     * @see #intersectsSegment(VSegment s, int jpx, int jpy, int tolerance, int
     * camIndex)
     */
    public List<VSegment> getIntersectingSegments(Camera c, int tolerance) {
     ...
     * camIndex)
     */
    public List<VSegment> getIntersectingSegments(Camera c, int x, int y, int tolerance) {
        List<VSegment> res = new ArrayList<VSegment>();
        int index = c.getIndex();
        List<Glyph> glyphs = c.getOwningSpace().getDrawnGlyphs(c.getIndex());
        Glyph glyph;
        for (int i = 0; i < glyphs.size(); i++) {
            glyph = glyphs.get(i);
            if ((glyph instanceof VSegment) && (intersectsSegment((VSegment)glyph, jpx, jpy, tolerance, index))) {
     ...
     *
     * @param camIndex indes of camera observing the segments of interest
     * (available through Camera.getIndex())
     * @param tolerance the segment's abstract thickness (w.r.t picking) in
     * pixels, not virtual space units (we consider a narrow rectangular region,
     * not an actual segment)
    ...
    /**
     * Get a list of all Glyphs (including segments and paths) picked. This
     * method is especially useful when the camera of interest is not the active
     * camera for the associated view (i.e. another layer is active). Beware of
     * the fact that this method returns glyphs of any kind, not just
     * ClosedShape instances. It can thus be much more computationaly expensive
     * than getpickedGlyphList()
     *
     * @param c a camera (the active camera can be obtained by
     * VirtualSpaceManager.getActiveCamera())
     * @return a list of glyphs under the mouse cursor, sorted by drawing order;
     * null if no object under the cursor.
     * @see #getPickedGlyphList()
     */
    public List<Glyph> getIntersectingGlyphs(Camera c) {
    ...
    /**
     * Double capacity of array containing glyphs under the cursor. Mechanism
     * similar to what Vectors do, bu we want to avoid casting.
     */
    void doubleCapacity() {
    ...
    /**
     * Get the list of glyphs currently picked. Last entry is last glyph
     * entered. This returns a <em>copy</em> of the actual array managed by the
     * picker at the time the method is called. In other words, the array
    ...
    /**
     * Remove glyph g in list of glyphs under picker if it is present. Called
     * when destroying a glyph.
     */
    void removeGlyphFromList(Glyph g) {
        ...
        if (present) {
            ...
            if (maxIndex < 0) {
                ...
                lastGlyphEntered = pickedGlyphs[maxIndex];
            }
        }
    }
...
//        }
    ...
    boolean checkGlyph(ViewListener eh, Camera c, int x, int y) {
        ...
        if (tmpRes == Glyph.ENTERED_GLYPH) {
            ...
            maxIndex = maxIndex + 1;
            if (maxIndex >= pickedGlyphs.length) {
                doubleCapacity();
            }
            ...
            maxIndex = maxIndex - 1;
            /*
             * required because list can be reset because we change layer and
             * then we exit a glyph
             */
            if (maxIndex < 0) {
                lastGlyphEntered = null;
                maxIndex = -1;
    ...
    /**
     * The list of glyphs under the picker and ordered according to the drawing
     * stack
     */
