/*
 * FILE: ViewPanel.java DATE OF CREATION: Jul 11 2000 AUTHOR : Emmanuel Pietriga
 ...
 *
 * $Id: ViewPanel.java 4598 2011-08-30 11:48:37Z epietrig $
 */
...
public abstract class ViewPanel implements MouseListener, MouseMotionListener, MouseWheelListener, ComponentListener {
    ...
    /**
     * draw no oval between point where we started dragging the mouse and
     * current point
     */
    public final static short NONE = 0;
    /**
     * draw an oval between point where we started dragging the mouse and
     * current point
     */
    public final static short OVAL = 1;
    /**
     * should a circle between point where we started dragging the mouse and
     * current point
     */
    ...
    /**
     * Repaintable means that this view should be repainted on a regular basis,
     * whenever it makes sense. This is false, e.g., when the view is iconified.
     */
    ...
    /**
     * graphics2d's original stroke -passed to each glyph in case it needs to
     * modifiy the stroke when painting itself
     */
    Stroke standardStroke;
    /**
     * graphics2d's original affine transform -passed to each glyph in case it
     * needs to modifiy the affine transform when painting itself
     */
    ...
    /**
     * Previous coordinates of the mouse. Used to erase old cursor before
     * repainting in XOR mode. Also used to resetMouseInsidePortals
     */
    ...
    /**
     * should we draw a line between point where we started dragging the mouse
     * and current point
     */
    boolean drawDrag = false;
    /**
     * should we draw a rectangle between point where we started dragging the
     * mouse and current point
     */
    boolean drawRect = false;
    /**
     * should we draw an oval between point where we started dragging the mouse
     * and current point
     */
    ...
    /**
     * Should the viewpanel automatically request focus when cursor enters it or
     * not.
     */
    ...
    protected void setAWTCursor(int cursorType) {
        if (cursorType == Cursor.CUSTOM_CURSOR) {
            /*
             * custom cursor is used to designate the VTM cursor. It is
             * transparent (cursor is painted at the end of each loop by the
             * view itself (as a cross))
             */
            ...
            /*
             * an AWT predefined cursor - forward the request to AWT and signal
             * ZVTM that it should not paint its own cursor
             */
            drawVTMcursor = false;
            try {
    ...
    /**
     * Set the cursor. Replaces the ZVTM cursor by a bitmap cursor similar to
     * the default AWT cursors.
     *
    ...
    /**
     * Converts between view coordinates and virtual space coordinates for a
     * given camera.
     *
     * @param c camera
    ...
    /**
     * Converts between virtual space coordinates and view coordinates for a
     * given camera.
     *
     * @param c camera
    ...
    /**
     * true will draw a segment between origin of drag and current cursor pos
     * until drag is finished (still visible for backward compatibility reasons
     * - should use setDrawSegment instead)
     */
    ...
    /**
     * true will draw a rectangle between origin of drag and current cursor pos
     * until drag is finished
     */
    ...
    /**
     * draw a circle between origin of drag and current cursor pos until drag is
     * finished (drag segment represents the radius of the circle, not its
     * diameter) - use OVAL for any oval, CIRCLE for circle, NONE to stop
     * drawing it
     */
    ...
    public void mousePressed(MouseEvent e) {
        ...
        origDragx = e.getX();
        origDragy = e.getY();  //store these anyway, since we have no way to know which button (if any) sets drawDrag mode
        if ((whichButton & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK) {
            if (e.isShiftDown()) {
                if (e.isControlDown()) {
                    evHs[activeLayer].press1(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e);
                } else if (e.isMetaDown()) {
                    evHs[activeLayer].press1(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e);
                } else if (e.isAltDown()) {
                    evHs[activeLayer].press1(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e);
                } else {
                    evHs[activeLayer].press1(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e);
                }
            } else if (e.isControlDown()) {
                evHs[activeLayer].press1(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e);
            } else {
                if (e.isMetaDown()) {
                    evHs[activeLayer].press1(this, ViewListener.META_MOD, e.getX(), e.getY(), e);
                } else if (e.isAltDown()) {
                    evHs[activeLayer].press1(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e);
                } else {
                    evHs[activeLayer].press1(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e);
                }
            }
        } else {
            if ((whichButton & InputEvent.BUTTON2_MASK) == InputEvent.BUTTON2_MASK) {
                if (e.isShiftDown()) {
                    if (e.isControlDown()) {
                        evHs[activeLayer].press2(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e);
                    } else if (e.isMetaDown()) {
                        evHs[activeLayer].press2(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e);
                    } else if (e.isAltDown()) {
                        evHs[activeLayer].press2(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e);
                    } else {
                        evHs[activeLayer].press2(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e);
                    }
                } else if (e.isControlDown()) {
                    evHs[activeLayer].press2(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e);
                } else {
                    if (e.isMetaDown()) {
                        evHs[activeLayer].press2(this, ViewListener.META_MOD, e.getX(), e.getY(), e);
                    } else if (e.isAltDown()) {
                        evHs[activeLayer].press2(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e);
                    } else {
                        evHs[activeLayer].press2(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e);
                    }
                }
            } else {
                if ((whichButton & InputEvent.BUTTON3_MASK) == InputEvent.BUTTON3_MASK) {
                    if (e.isShiftDown()) {
                        if (e.isControlDown()) {
                            evHs[activeLayer].press3(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e);
                        } else if (e.isMetaDown()) {
                            evHs[activeLayer].press3(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e);
                        } else if (e.isAltDown()) {
                            evHs[activeLayer].press3(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e);
                        } else {
                            evHs[activeLayer].press3(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e);
                        }
                    } else if (e.isControlDown()) {
                        evHs[activeLayer].press3(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e);
                    } else {
                        if (e.isMetaDown()) {
                            evHs[activeLayer].press3(this, ViewListener.META_MOD, e.getX(), e.getY(), e);
                        } else if (e.isAltDown()) {
                            evHs[activeLayer].press3(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e);
                        } else {
                            evHs[activeLayer].press3(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e);
                        }
                    }
                }
            }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if ((whichButton & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK) {
            if (e.isShiftDown()) {
                if (e.isControlDown()) {
                    evHs[activeLayer].click1(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                } else if (e.isMetaDown()) {
                    evHs[activeLayer].click1(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                } else if (e.isAltDown()) {
                    evHs[activeLayer].click1(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                } else {
                    evHs[activeLayer].click1(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                }
            } else if (e.isControlDown()) {
                evHs[activeLayer].click1(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e.getClickCount(), e);
            } else {
                if (e.isMetaDown()) {
                    evHs[activeLayer].click1(this, ViewListener.META_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                } else if (e.isAltDown()) {
                    evHs[activeLayer].click1(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                } else {
                    evHs[activeLayer].click1(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e.getClickCount(), e);
                }
            }
        } else {
            if ((whichButton & InputEvent.BUTTON2_MASK) == InputEvent.BUTTON2_MASK) {
                if (e.isShiftDown()) {
                    if (e.isControlDown()) {
                        evHs[activeLayer].click2(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                    } else if (e.isMetaDown()) {
                        evHs[activeLayer].click2(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                    } else if (e.isAltDown()) {
                        evHs[activeLayer].click2(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                    } else {
                        evHs[activeLayer].click2(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                    }
                } else if (e.isControlDown()) {
                    evHs[activeLayer].click2(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                } else {
                    if (e.isMetaDown()) {
                        evHs[activeLayer].click2(this, ViewListener.META_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                    } else if (e.isAltDown()) {
                        evHs[activeLayer].click2(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                    } else {
                        evHs[activeLayer].click2(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e.getClickCount(), e);
                    }
                }
            } else {
                if ((whichButton & InputEvent.BUTTON3_MASK) == InputEvent.BUTTON3_MASK) {
                    if (e.isShiftDown()) {
                        if (e.isControlDown()) {
                            evHs[activeLayer].click3(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                        } else if (e.isMetaDown()) {
                            evHs[activeLayer].click3(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                        } else if (e.isAltDown()) {
                            evHs[activeLayer].click3(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                        } else {
                            evHs[activeLayer].click3(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                        }
                    } else if (e.isControlDown()) {
                        evHs[activeLayer].click3(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                    } else {
                        if (e.isMetaDown()) {
                            evHs[activeLayer].click3(this, ViewListener.META_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                        } else if (e.isAltDown()) {
                            evHs[activeLayer].click3(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e.getClickCount(), e);
                        } else {
                            evHs[activeLayer].click3(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e.getClickCount(), e);
                        }
                    }
                }
            }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if ((whichButton & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK) {
            if (e.isShiftDown()) {
                if (e.isControlDown()) {
                    evHs[activeLayer].release1(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e);
                } else if (e.isMetaDown()) {
                    evHs[activeLayer].release1(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e);
                } else if (e.isAltDown()) {
                    evHs[activeLayer].release1(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e);
                } else {
                    evHs[activeLayer].release1(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e);
                }
            } else if (e.isControlDown()) {
                evHs[activeLayer].release1(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e);
            } else {
                if (e.isMetaDown()) {
                    evHs[activeLayer].release1(this, ViewListener.META_MOD, e.getX(), e.getY(), e);
                } else if (e.isAltDown()) {
                    evHs[activeLayer].release1(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e);
                } else {
                    evHs[activeLayer].release1(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e);
                }
            }
        } else {
            if ((whichButton & InputEvent.BUTTON2_MASK) == InputEvent.BUTTON2_MASK) {
                if (e.isShiftDown()) {
                    if (e.isControlDown()) {
                        evHs[activeLayer].release2(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e);
                    } else if (e.isMetaDown()) {
                        evHs[activeLayer].release2(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e);
                    } else if (e.isAltDown()) {
                        evHs[activeLayer].release2(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e);
                    } else {
                        evHs[activeLayer].release2(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e);
                    }
                } else if (e.isControlDown()) {
                    evHs[activeLayer].release2(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e);
                } else {
                    if (e.isMetaDown()) {
                        evHs[activeLayer].release2(this, ViewListener.META_MOD, e.getX(), e.getY(), e);
                    } else if (e.isAltDown()) {
                        evHs[activeLayer].release2(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e);
                    } else {
                        evHs[activeLayer].release2(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e);
                    }
                }
            } else {
                if ((whichButton & InputEvent.BUTTON3_MASK) == InputEvent.BUTTON3_MASK) {
                    if (e.isShiftDown()) {
                        if (e.isControlDown()) {
                            evHs[activeLayer].release3(this, ViewListener.CTRL_SHIFT_MOD, e.getX(), e.getY(), e);
                        } else if (e.isMetaDown()) {
                            evHs[activeLayer].release3(this, ViewListener.META_SHIFT_MOD, e.getX(), e.getY(), e);
                        } else if (e.isAltDown()) {
                            evHs[activeLayer].release3(this, ViewListener.ALT_SHIFT_MOD, e.getX(), e.getY(), e);
                        } else {
                            evHs[activeLayer].release3(this, ViewListener.SHIFT_MOD, e.getX(), e.getY(), e);
                        }
                    } else if (e.isControlDown()) {
                        evHs[activeLayer].release3(this, ViewListener.CTRL_MOD, e.getX(), e.getY(), e);
                    } else {
                        if (e.isMetaDown()) {
                            evHs[activeLayer].release3(this, ViewListener.META_MOD, e.getX(), e.getY(), e);
                        } else if (e.isMetaDown()) {
                            evHs[activeLayer].release3(this, ViewListener.ALT_MOD, e.getX(), e.getY(), e);
                        } else {
                            evHs[activeLayer].release3(this, ViewListener.NO_MODIFIER, e.getX(), e.getY(), e);
                        }
                    }
                }
            }
    ...
    /**
     * Should the viewpanel automatically request focus when cursor enters it or
     * not. Default is false for external views (EView). Default is true for
     * panel views (PView) as keyboard events don't get sent otherwise.
     */
    ...
    /**
     * Tells whether the viewpanel is automatically requesting focus when cursor
     * enters it or not. Default is false for external views (EView). Default is
     * true for panel views (PView) as keyboard events don't get sent otherwise.
     */
    ...
    /**
     * Value that specifies that there isn't any point for which no mouse
     * move/drag event is sent.
     */
    public static final int NO_COORDS = -1;
    int ix, iy = NO_COORDS;

    /**
     * Set a particular point in view panel coordinates for which mouse
     * move/drag events should be ignored. This is useful, e.g., to completely
     * ignore mouse repositioning events triggered by an AWT Robot.
     *
     * @param x x-coordinate in the JPanel's system ; set to ViewPanel.NO_COORDS
     * to cancel any previsouly set point.
     * @param y y-coordinate in the JPanel's system ; set to ViewPanel.NO_COORDS
     * to cancel any previsouly set point.
     */
    ...
    /**
     * Get the coordinates of a particular point in view panel coordinates for
     * which mouse move/drag events are ignored, if any. Such a point is useful,
     * e.g., to completely ignore mouse repositioning events triggered by an AWT
     * Robot.
     *
     * @return (x,y) coordinates of the point in the JPanel's system, null if no
     * such point is set.
     */
            ...
            if (parent.mouse.sync && (e.getX() != ix || e.getY() != iy)) {
                ...
                if (evHs[activeLayer] != null) {
                    ...
                    if (e.isShiftDown()) {
                        ...
                        if (e.isControlDown()) {
                            evHs[activeLayer].mouseDragged(this, ViewListener.CTRL_SHIFT_MOD, buttonNumber, e.getX(), e.getY(), e);
                        } else if (e.isMetaDown()) {
                            evHs[activeLayer].mouseDragged(this, ViewListener.META_SHIFT_MOD, buttonNumber, e.getX(), e.getY(), e);
                        } else if (e.isAltDown()) {
                            evHs[activeLayer].mouseDragged(this, ViewListener.ALT_SHIFT_MOD, buttonNumber, e.getX(), e.getY(), e);
                        } else {
                            evHs[activeLayer].mouseDragged(this, ViewListener.SHIFT_MOD, buttonNumber, e.getX(), e.getY(), e);
                        }
                    } else if (e.isControlDown()) {
                        evHs[activeLayer].mouseDragged(this, ViewListener.CTRL_MOD, buttonNumber, e.getX(), e.getY(), e);
                    } else {
                        if (e.isMetaDown()) {
                            evHs[activeLayer].mouseDragged(this, ViewListener.META_MOD, buttonNumber, e.getX(), e.getY(), e);
                        } else if (e.isAltDown()) {
                            evHs[activeLayer].mouseDragged(this, ViewListener.ALT_MOD, buttonNumber, e.getX(), e.getY(), e);
                        } else {
                            evHs[activeLayer].mouseDragged(this, ViewListener.NO_MODIFIER, buttonNumber, e.getX(), e.getY(), e);
                        }
                    }
                }
    ...
    /**
     * Get the list of glyphs currently under mouse (last entry is last glyph
     * entered) This returns a <em>copy</em> of the actual array managed by
     * VCursor at the time the method is called (in other words, the array
     * returned by this method is not synchronized with the actual list over
     * time)
     */
    ...
    /**
     * Sets the maximum view refresh rate by giving the minimum refresh period
     * (below which ZVTM won't go even if it can)
     *
    ...
    /**
     * Gets the maximum view refresh rate as the minimum refresh period (below
     * which ZVTM won't go even if it can)
     *
