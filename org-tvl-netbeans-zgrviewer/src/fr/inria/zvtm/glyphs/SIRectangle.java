/*
 * FILE: SIRectangle.java DATE OF CREATION: Thu Dec 05 13:53:36 2002 AUTHOR :
...
/**
 * Rectangle with constant apparent width and height, no matter the camera's
 * altitude. Used for instance to create resizing handles. Cannot be reoriented.
 *
 * @author Emmanuel Pietriga
 *
 */
public final class SIRectangle extends ClosedShape implements RectangularShape {
    ...
    public boolean visibleInRegion(double wb, double nb, double eb, double sb, int i) {
        if ((vx >= wb) && (vx <= eb) && (vy >= sb) && (vy <= nb)) {
            ...
            if (((vx - pc[i].cw) <= eb) && ((vx + pc[i].cw) >= wb) && ((vy - pc[i].ch) <= nb) && ((vy + pc[i].ch) >= sb)) {
                /*
                 * Glyph is at least partially in region. We approximate using
                 * the glyph bounding box, meaning that some glyphs not actually
                 * visible can be projected and drawn (but they won't be
                 * displayed))
                 */
                return true;
            }
    ...
    public boolean fillsView(double w, double h, int camIndex) {//width and height of view - pc[i].c? are JPanel coords
        if ((w <= pc[camIndex].cx + pc[camIndex].cw) && (0 >= pc[camIndex].cx - pc[camIndex].cw) && (h <= pc[camIndex].cy + pc[camIndex].ch) && (0 >= pc[camIndex].cy - pc[camIndex].ch)) {
            return true;
        } else {
            return false;
        }
