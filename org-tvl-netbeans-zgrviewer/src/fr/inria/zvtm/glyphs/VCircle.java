/*
 * FILE: VCircle.java DATE OF CREATION: Nov 22 2000 AUTHOR : Emmanuel Pietriga
...
public class VCircle extends ClosedShape {
    ...
    public boolean fillsView(double w, double h, int camIndex) {
        if ((alphaC == null) && (Math.sqrt(Math.pow(w - pc[camIndex].cx, 2) + Math.pow(h - pc[camIndex].cy, 2)) <= pc[camIndex].cr / 2d)
            ...
            && (Math.sqrt(Math.pow(pc[camIndex].cx, 2) + Math.pow(pc[camIndex].cy, 2)) <= pc[camIndex].cr / 2d)) {
            return true;
        } else {
            return false;
        }
