/*
 * FILE: Glyph.java DATE OF CREATION: Jul 11 2000 AUTHOR : Emmanuel Pietriga
...
public abstract class Glyph implements Cloneable, Translucent {
     ...
     */
    /**
     * Object this glyph represents in the client application. The owner can be
     * any arbitrary Java object set by the client application, that the
     * programmer wants to be easily accessible through the Glyph (typically the
     * owner will be an object that models a logical concept of which a visual
     * depiction is given by the glyph). Multiple glyphs can have the same
     * owner. A glyph can only have one owner. Set by client application. Null
     * if not set.
     */
    private Object owner;
    /**
     * Type of object. Arbitrary String, set by client application. Null if not
     * set.
     */
    private String type;

    /**
     * Get the object this glyph represents in the client application. The owner
     * can be any arbitrary Java object set by the client application, that the
     * programmer wants to be easily accessible through the Glyph (typically the
     * owner will be an object that models a logical concept of which a visual
     * depiction is given by the glyph). Multiple glyphs can have the same
     * owner. A glyph can only have one owner.
     *
    ...
    /**
     * Set the object this glyph represents in the client application. The owner
     * can be any arbitrary Java object set by the client application, that the
     * programmer wants to be easily accessible through the Glyph (typically the
     * owner will be an object that models a logical concept of which a visual
     * depiction is given by the glyph). Multiple glyphs can have the same
    ...
    /**
     * Get the type of this glyph. Arbitrary String, set by client application.
     * Null if not set. This is somewhat equivalent to tagging an object, but it
     * only one type can be associated with a glyph.
     *
    ...
    /**
     * Set the type of this glyph. This is somewhat equivalent to tagging an
     * object, but it only one type can be associated with a glyph.
     *
    ...
    public String toString() {
        return super.toString() + " Glyph ID " + hashCode() + " pos (" + vx + "," + vy + "," + vz + ") type=" + type;
    }
     ...
     */
    /**
     * Horizontal coordinate of the glyph's geomatrical center, in virtual
     * space.
     *
     ...
     *
     * @return a copy of the glyph's location. Changing the x,y coordinates of
     * the returned point will not have any effect on the glyph's position.
     */
    ...
    /**
     * FOR INTERNAL USE ONLY. Will have no effect on actual z-ordering. Use
     * appropriate methods in VirtualSpace.
     *
    ...
    /**
     * Indicates whether ZVTM sends events related to cursor entry/exit in/from
     * this glyph or not.
     */
    ...
    /**
     * Make this glyph (in)visible (the glyph remains sensitive to cursor in/out
     * events).<br> Use methods VirtualSpace.show(Glyph g) and
     * VirtualSpace.hide(Glyph g) to make a glyph both (in)visible and
     * (in)sensitive.
     *
    ...
    /**
     * Get this glyph's visibility status when seen through a lens. This is
     * equivalent to isVisible() for most glyphs, except LText.
     *
     * @return true if visible
     */
    ...
    /**
     * Indicates whether this glyph's interior is filled or not. Relevant for
     * closed shapes only. Does not make sense for glyphs such as text, paths
     * and segments (returns true in those cases).
     */
    ...
    /**
     * Indicates whether the glyph's border is drawn or not. Relevant for closed
     * shapes only. Does not make sense for glyphs such as text, paths and
     * segments (returns true in those cases).
     */
    ...
    /**
     * Set the glyph's border color when cursor is inside it. IMPORTANT: add ing
     * the glyph to a virtual space <strong>after</strong> calling this method
     * might override this color. Relevant for closed shapes only. Does not make
     * sense for glyphs such as text, paths and segments.
     *
    ...
    /**
     * Set the glyph's main color. This is the fill color for closed shapes, or
     * stroke color for other glyphs (text, paths, segments, etc.).
     */
    ...
    /**
     * Set the glyph's border color (use setColor for text, paths, segments,
     * etc.). This will have an effect only for Glyphs that have a border, such
     * as instances of ClosedShape.
     *
    ...
    /**
     * Set the glyph's border color (absolute value, HSV color space). Use
     * setColor for text, paths, segments, etc. This will have an effect only
     * for Glyphs that have a border, such as instances of ClosedShape.
     *
     * @param h hue in [0.0, 1.0]
     * @param s saturation in [0.0, 1.0]
     * @param v value (brightness) in [0.0, 1.0]
    ...
    /**
     * Get border color's HSV components. This is the original array, not a
     * copy.
     *
     * @return {0, 0, 0} if the glyph does not have a border (e.g., is not an
     * instanceof ClosedShape)
     */
    ...
    /**
     * Get the glyph's current main color. This might be different from the
     * default main color depending on the Glyph's current status. This is the
     * fill color for closed shapes, or stroke color for other glyphs (text,
     * paths, segments, etc.).
     *
     * @see #getDefaultColor()
     */
    ...
    /**
     * Get the glyph's current main color. This might be different from the
     * current main color depending on the Glyph's current status. This is the
     * fill color for closed shapes, or stroke color for other glyphs (text,
     * paths, segments, etc.).
     *
     * @see #getColor()
    ...
    /**
     * Get the glyph's border color (use getColor for text, paths, segments,
     * etc.).
     *
     * @return Color.BLACK if the glyph does not have a border (e.g., is not an
     * instanceof ClosedShape)
     * @see #getColor()
     */
    ...
    /**
     * Highlight this glyph to give visual feedback when the cursor is inside
     * it.
     */
     ...
     */
    /**
     * AlphaComposite used to paint glyph if not opaque. Set to null if glyph is
     * opaque. Temporarily made public until all glyphs get into the same
     * package.
     */
    ...
    Glyph[] stickedGlyphs;
    /**
     * Object to which this glyph is sticked (could be a VCursor, a Camera or a
     * Glyph).
     */
    ...
    /**
     * Attach a glyph to this one. Translations of this glyph will be propagated
     * to g.
     *
    ...
    /**
     * Unattach a glyph from this one. Translations of this glyph will no longer
     * be propagated to g.
     *
    ...
    /**
     * Unattach all glyphs attached to this one. Translations of this glyph will
     * no longer be propagated to them.
     */
    ...
    /**
     * Value sent to cursor when it neither enters nor exit this glyph. For
     * internal use.
     */
    ...
    /**
     * Project glyph w.r.t a given camera's coordinate system, prior to actual
     * painting. Called internally.
     *
     * @param c camera
    ...
    /**
     * Project glyph w.r.t a given camera's coordinate system, prior to actual
     * painting through a lens. Called internally.
     *
     * @param c camera
     ...
     * @param lensMag magnification factor of lens activated in View using this
     * camera
     * @param lensx horizontal coordinateof lens activated in View using this
     * camera
     * @param lensy vertical coordinate of lens activated in View using this
     * camera
     */
     ...
     * @param vW associated View width (used by some closed shapes to determine
     * if it is worth painting the glyph's border)
     * @param vH associated View height (used by some closed shapes to determine
     * if it is worth painting the glyph's border)
    ...
    /**
     * Initialize projected coordinates. Called internally when glyph is created
     * in order to create the initial set of projected coordinates w.r.t the
     * number of cameras in the virtual space.
     *
    ...
    /**
     * Create new projected coordinates. Called internally to create new
     * projected coordinates to use with the new camera.
     *
     * @param verifIndex camera index, just to be sure that the number of
     * projected coordinates is consistent with the number of cameras.
    ...
    /**
     * Dispose of projected coordinates. If a camera is removed from the virtual
     * space to which this glyphs belongs, the corresponding projected
     * coordinates should be deleted. the array of projected coordinates should
     * not be modified however, because other cameras' index remain the same.
     * The corresponding index in the array should just be set to null.
     *
     * @see #initCams(int nbCam)
     * @see #addCamera(int verifIndex)
     */
     ...
     *
     * @param jpx provide projected JPanel coordinates of the associated view,
     * not virtual space coordinates
     * @param jpy provide projected JPanel coordinates of the associated view,
     * not virtual space coordinates
    ...
    /**
     * Reset memory of cursor being inside the glyph for projected coordinates
     * associated with camera at index i.
     */
     ...
     * @param cvx virtual space coordinates
     * @param cvy virtual space coordinates
     * @return VCursor.ENTERED_GLYPH if cursor has entered the glyph,
     * VCursor.EXITED_GLYPH if it has exited the glyph, VCursor.NO_CURSOR_EVENT
     * if nothing has changed (meaning the cursor was already inside or outside
     * it)
     */
    public abstract short mouseInOut(int jpx, int jpy, int camIndex, double cvx, double cvy);

    /**
     * Method used internally to find out if it is necessary to project and draw
     * this glyph for a given camera.
     *
     * @return true if the glyph is currently visible in the region delimited by
     * wb, nb, eb, sb, symbolising the region seen through a camera
     ...
     * @param i camera index (useuful only for some glyph classes redefining
     * this method)
     */
    public boolean visibleInRegion(double wb, double nb, double eb, double sb, int i) {
        if ((vx >= wb) && (vx <= eb) && (vy >= sb) && (vy <= nb)) {
            ...
            /*
             * Glyph is at least partially in region. We approximate using the
             * glyph bounding box, meaning that some glyphs not actually visible
             * can be projected and drawn (but they won't be displayed))
             */
            return true;
        }
        return false;
    ...
    /**
     * Method used internally to find out if it is necessary to project and draw
     * the glyph through a lens for a given camera.
     *
     * @param wb west region boundary (virtual space coordinates)
     * @param nb north region boundary (virtual space coordinates)
     * @param eb east region boundary (virtual space coordinates)
     ...
     * @param i camera index (useuful only for some glyph classes redefining
     * this method)
     * @return true if the glyph intersects the region delimited by wb, nb, eb,
     * sb
     */
    public boolean containedInRegion(double wb, double nb, double eb, double sb, int i) {
        if ((vx >= wb) && (vx <= eb) && (vy >= sb) && (vy <= nb)) {
            ...
            if (((vx + size) <= eb) && ((vx - size) >= wb) && ((vy + size) <= nb) && ((vy - size) >= sb)) {
                ...
                return false;   //otherwise the glyph is not visible
            }
        }
        return false;
    }
    ...
    /**
     * Find out if this glyph completely fills a view. (In which case it is not
     * necessary to repaint objects below it in the drawing stack). If
     * implemented, this method should be very efficient, as it is used by an
     * optional top-down clipping algorithm. Otherwise it might cost more time
     * than it can potentially save. Until now it has only been implemented for
     * non-reorientable rectangles and was activated only for treemap-like
     * representations in which a lot of rectangles can potentially overlap each
     * other.
     */
    public abstract boolean fillsView(double w, double h, int camIndex);

    /**
     * Find out if a glyph is visible in a circular area. Not implemented yet
     * for most glyph classes. Used for instance to implement DynaSpot picking.
     */
     ...
     */
    /**
     * Get the Java2D Shape corresponding to this Glyph. Virtual Space
     * coordinates.
     */
