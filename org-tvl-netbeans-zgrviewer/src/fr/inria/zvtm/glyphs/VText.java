/*
 * FILE: VText.java DATE OF CREATION: Nov 23 2000 AUTHOR : Emmanuel Pietriga
...
/**
 * Standalone Text. This version is the most efficient, but it cannot be
 * reoriented (see VTextOr*).<br> Font properties are set globally in the view,
 * but can be changed on a per-instance basis using setSpecialFont(Font f).<br>
 * (vx, vy) are the coordinates of the lower-left corner, or lower middle point,
 * or lower-right corner depending on the text anchor (start, middle, end).
 *
...
public class VText extends ClosedShape {
    ...
    /**
     * Text alignment (for text anchor) used to align a VText (vx,vy coordinates
     * coincides with start of String).
     */
    public static final short TEXT_ANCHOR_START = 0;
    /**
     * Text alignment (for text anchor) used to align a VText (vx,vy coordinates
     * coincides with middle of String).
     */
    public static final short TEXT_ANCHOR_MIDDLE = 1;
    /**
     * Text alignment (for text anchor) used to align a VText (vx,vy coordinates
     * coincides with end of String).
     */
    ...
    /**
     * Set to false if the text should not be scaled according to camera's
     * altitude. Its apparent size will always be the same, no matter the
     * camera's altitude.
     *
     * @see #isZoomSensitive()
     */
    ...
    /**
     * Set the value under which a VText is drawn as a point/segment instead of
     * an actual text (considered too small to be read).
     *
     * @param f value compared to the product of the font size by the projection
     * value. Default is 0.5. Raising this value implies that more text that was
     * still displayed as a string will be displayed as a segment and
     * conversely.
     */
    ...
    /**
     * Get the value under which a VText is drawn as a point/segment instead of
     * an actual text (considered too small to be read).
     *
     * @return the value compared to the product of the font size by the
     * projection value. Default is 0.5. Raising this value implies that more
     * text that was still displayed as a string will be displayed as a segment
     * and conversely.
     */
    ...
    /**
     * Set the scale factor for this text. The actual size of the text will be
     * that defined by the font size multiplied by this scale factor.
     *
    ...
    /**
     * Get the scale factor for this text. The actual size of the text is that
     * defined by the font size multiplied by the scale factor returned by this
     * method.
     */
     ...
     * The bounds can be invalid if the thread in charge of painting has not
     * dealt with this glyph since invalidate() was last called on it. It is
     * advisable to test this before calling getBounds(int i)
     *
     * @see #getBounds(int i)
     * @see #invalidate()
     */
    ...
    /**
     * Indicates whether this glyph is using a special font. Special means
     * different from the default one declared in the VirtualSpaceManager.
     *
     * @see #setFont(Font f)
     * @see #getFont()
     */
    ...
    public void highlight(boolean b, Color selectedColor) {
        ...
        if (b) {
            if (cursorInsideFColor != null) {
                color = cursorInsideFColor;
                update = true;
            }
            if (cursorInsideColor != null) {
                borderColor = cursorInsideColor;
                update = true;
            }
        } else {
