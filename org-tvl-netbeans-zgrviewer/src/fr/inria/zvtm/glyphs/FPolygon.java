/*
 * FILE: FPolygon.java DATE OF CREATION: Mon Jan 13 13:34:44 2003 AUTHOR :
 * Emmanuel Pietriga (emmanuel@w3.org) MODIF: Emmanuel Pietriga
 * (emmanuel.pietriga@inria.fr) Copyright (c) Emmanuel Pietriga, 2002. All
 * Rights Reserved Copyright (c) INRIA, 2004-2010. All Rights Reserved Licensed
 * under the GNU LGPL. For full terms see the file COPYING.
 *
 * $Id: FPolygon.java 4494 2011-06-13 11:15:54Z epietrig $
 */
...
/**
 * Fast Polygon. Can neither be resized nor reoriented (for now). This is the
 * old implementation of VPolygon, as found in ZVTM 0.8.2.<br> The new version
 * of VPolygon can be resized, but at some cost from an efficiency point of
 * view, so the old version is still provided here and can be used by people who
 * do not intend to resize their Polygon instances.<br> This implementation uses
 * doubles instead of doubles for its internal representation of the vertices,
 * see VPolygon for more details.
 *
...
public class FPolygon extends ClosedShape {
    ...
    /*
     * array of projected coordinates - index of camera in virtual space is
     * equal to index of projected coords in this array
     */
    ProjPolygon[] pc;
    ...
    public void addCamera(int verifIndex) {
        if (pc != null) {
            if (verifIndex == pc.length) {
                ...
                System.err.println("FPolygon:Error while adding camera " + verifIndex);
            }
        } else {
            if (verifIndex == 0) {
    ...
    public boolean fillsView(double w, double h, int camIndex) {
        if ((alphaC == null)
            && (pc[camIndex].p.contains(0, 0)) && (pc[camIndex].p.contains(w, 0)) && (pc[camIndex].p.contains(0, h)) && (pc[camIndex].p.contains(w, h))) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean coordInside(int jpx, int jpy, int camIndex, double cvx, double cvy) {
        if (pc[camIndex].p.contains(jpx, jpy)) {
            return true;
        } else {
            return false;
        }
     ...
     *
     * @return a semicolon-separated string representation of all vertex
     * absolute coordinates (x and y coordinates seperated by commas, e.g.
     * x1,y1;x2,y2;x3,y3 etc.)
     */
    public String getVerticesAsText() {
        StringBuilder res = new StringBuilder();
        for (int i = 0; i < xcoords.length - 1; i++) {
            res.append(xcoords[i]).append(vx).append(",").append(ycoords[i]).append(vy).append(";");
        }
        res.append(xcoords[xcoords.length - 1]).append(vx).append(",").append(ycoords[ycoords.length - 1]).append(vy);
        return res.toString();
    }
     ...
     * coordinates. <strong>Warning:</strong> the polygon's coordinates are
     * converted to 32-bit integers for now (we use a java.awt.Polygon)
     */
    @Override
