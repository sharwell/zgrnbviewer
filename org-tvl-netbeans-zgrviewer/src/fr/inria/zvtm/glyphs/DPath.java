/*
 * FILE: DPath.java DATE OF CREATION: Thu Mar 29 19:33 2007 AUTHOR : Emmanuel
 * Pietriga (emmanuel.pietriga@inria.fr) Copyright (c) INRIA, 2007-2011. All
...
/**
 * Dynamic Path, made of an arbitrary number of segments, quadratic curves,
 * cubic curves, and gaps. All of these can be dynamically modified and animated
 * through AnimManager's createPathAnimation method. This class implements
 * RectangularShape even though it is not, just to give easy access to the
 * path's bounding box dimensions.
 *
...
public class DPath extends Glyph implements RectangularShape {
    ...
    /**
     * For internal use. Dot not tamper with. Made public for outside package
     * subclassing. Stores projected start point only.
     */
    ...
    /*
     * vx,vy represent the path's hotspot, i.e., the center of the bounding box,
     * not necessarily on the path itself
     */
    ...
    double spx, spy;
    /**
     * Width in virtual space. For internal use. Made public for easier outside
     * package subclassing.
     */
    public double vw;
    /**
     * Height in virtual space. For internal use. Made public for easier outside
     * package subclassing.
     */
    public double vh;
    /**
     ...
     * @param z z-index (pass 0 if you do not use z-ordering)
     * @param c color
     * @param alpha alpha channel value in [0;1.0] 0 is fully transparent, 1 is
     * opaque
     */
    ...
    /**
     * Add a new cubic curve to the path, from current point to point (x,y),
     * controlled by (x1,y1)
     *
     ...
     * @param abs true if coordinates should be interpreted as absolute
     * coordinates, false if coordinates should be interpreted as relative
     * coordinates (w.r.t last point)
     */
    ...
    /**
     * Add a new quadratic curve to the path, from current point to point (x,y),
     * controlled by (x1,y1)
     *
     * @param x x coordinate of end point in virtual space
     * @param y y coordinate of end point in virtual space
    ...
    /**
     * Add a new 'gap' to the path (move without drawing anything), from current
     * point to point (x,y).
     *
     * @param x x coordinate of end point in virtual space
     * @param y y coordinate of end point in virtual space
     ...
     */
    /**
     * Get the horizontal distance from western-most point to the eastern-most
     * one.
     */
    @Override
    ...
    /**
     * Get the vertical distance from northern-most point to the southern-most
     * one.
     */
    @Override
    ...
    /**
     * Draw the general path that gets compiled each time we make a change to
     * the DPath (default).
     */
    public static final short DRAW_GENERAL_PATH = 0;
    /**
     * Iterate on the path's components (each line, quad curve, cubic curve) and
     * draw them step by step.
     */
     ...
     *
     * @param m method to draw this glyph. <ul> <li> DRAW_GENERAL_PATH will draw
     * the general path that gets compiled each time we make a change to the
     * DPath (default).</li> <li> DRAW_SUBPATHS will iterate on the path's
     * components (each line, quad curve, cubic curve) and draw them step by
     * step.</li> </ul>
     */
    ...
    /**
     * The disc is actually approximated to its bounding box here. Precise
     * intersection computation would be too costly.
     */
    @Override
     ...
     * @param ctrlPoints list of the points that contain coordinates of the
     * control point(s) (in case of QD/CB curve)
     ...
     *
     * @param points List of new coordinates for each point. Example order could
     * be: startPoint, controlPoint1, controlPoint2, endPoint, controlPoint1,
     * endPoint, endPoint ...
     ...
     *
     * @param index index of the element in the DPath
     * @return -1 in case of incorrect parameters, otherwise returns one of the
     * constants DPath.CBC, DPath.QDC, DPath.SEG, DPath.MOV
     */
     ...
     *
     * @param index index of the element in the DPath
     * @return List of element's points ordered as startPoint, controlPoint1,
     ...
     *
     * @return a copy of the list of points in following format: startPoint,
    ...
    /**
     * Calculates coordinates of all DPath's points (including control points)
     * to display the DPath as a line.
     *
     ...
     * @return List of LongPoint absolute coordinates that can be passed to the
     * edit(LongPoint[], boolean) method or to the AnimManager
     */
    ...
    /**
     * Get the Java2D GeneralPath representing this DPath in virtual space
     * coordinates.
     */
