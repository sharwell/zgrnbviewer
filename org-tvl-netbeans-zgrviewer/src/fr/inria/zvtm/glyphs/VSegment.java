/*
 * FILE: VSegment.java DATE OF CREATION: Jul 20 2000 AUTHOR : Emmanuel Pietriga
 ...
 * Computing, 2002. All Rights Reserved Copyright (c) INRIA, 2004-2011. All
 * Rights Reserved
 *
 ...
 */
public final class VSegment extends Glyph implements RectangularShape {
    ...
    /**
     * Change the segment's location, size and orientation by giving its two
     * endpoints (absolute coordinates).
     */
    ...
    void computeSize() {
        ...
        if (orient < 0) {
            ...
            if (vh > 0) {
                orient = 2 * Math.PI - orient;
            } else {
                orient = Math.PI - orient;
            }
        } else if (orient == 0 && vw < 0) {
            orient = Math.PI;
        }
    }
    ...
    /**
     * Detects whether the point (x,y) lies on the segment or not. Default
     * tolerance of 2 pixels.
     *
     * @param x EXPECTS PROJECTED JPanel COORDINATE (obtained e.g. in
     * ViewListener's mouse methods as jpx)
     * @param y EXPECTS PROJECTED JPanel COORDINATE (obtained e.g. in
     * ViewListener's mouse methods as jpy)
     ...
     * @param y EXPECTS PROJECTED JPanel COORDINATE (obtained e.g. in
     * ViewListener's mouse methods as jpy)
     * @param tolerance the segment's clickable thickness in pixels, not virtual
     * space units
     * @param camIndex camera index (obtained through Camera.getIndex())
     */
